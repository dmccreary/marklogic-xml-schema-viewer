...
declare function svg-util:split-long-text($input as xs:string) as xs:string* {
let $words := tokenize($input, ' ')
let $count := count($words)
let $lines := $count div 20
return
for $sentence in (1 to $lines)
  for $word in $words[$sentence to ($sentence + 20)]
   return
      $word
 };
